...
Boolean streamUsingTCP = True; // TRUE for 'control' over TCP. Test for strem over internet.
...
unsigned qosMeasurementIntervalMS = 5.0; // 0 means: Don't output QOS data
...
bool usage(int count, char **argv)
{
    if (count == 1)
    {
        std::cerr << "Usage StreamerRTSP : ./streamerClientRTSP <url> /* with URL format like rtsp://URL/filename */" << std::endl;
        return false;
    }
    return true;
}
...
void continueAfterPLAY(RTSPClient*, int resultCode, char* resultString)
{
    ...
    if (qosMeasurementIntervalMS > 0)
    {
       ...
       beginQOSMeasurement(session, qosMeasurementTimerTask, qosMeasurementIntervalMS, env);
    }
...
static void scheduleNextQOSMeasurement(TaskToken qosMeasurementTimerTask, unsigned int qosMeasurementIntervalMS, UsageEnvironment *env)
{
